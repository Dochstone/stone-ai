...
# ═══════════════════════════════════════════════════════════
# Текстовый чат — просто пишешь боту → AI отвечает
# ═══════════════════════════════════════════════════════════
...
async def text_chat(message: Message):
    ...
    if tier == "free" and balance <= 0:
        used = user.daily_lite_used or 0
        if used >= 15:
            await message.answer("🔒 Лимит запросов исчерпан (15/день). Подписка от 390₽/мес → /plan")
            return
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            if full_text:
                ...
                async with async_session() as db:
                    ...
                    u = result.scalar_one_or_none()
                    if u:
                        u.daily_lite_used = (u.daily_lite_used or 0) + 1
                        u.total_requests = (u.total_requests or 0) + 1
                        await db.commit()
            else:
                await wait_msg.edit_text("❌ Пустой ответ от модели.")
