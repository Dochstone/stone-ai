...
ALLOWED_HEALTH_MODELS = {"claude-opus-4.5", "claude-opus-4-7", "gpt-5.5", "gemini-3.1-pro-preview"}
HEALTH_SCENARIOS = (
    "general",
    "skin",
    "eyes",
...
HEALTH_RESPONSE_MODES = ("short", "balanced", "detailed", "doctor", "surgeon", "lor", "dentistry")
...
SCENARIO_GUIDES: dict[str, str] = {
    ...
    "teeth": (
        "Фокус: зубы и область вокруг них. Оцени кариозные дефекты, сколы, потемнение, воспаление дёсен, "
        "отёк, боль при накусывании, оголение шейки зуба, налёт и признаки абсцесса."
    ),
    "throat": (
        "Фокус: горло, миндалины, задняя стенка глотки, покраснение, налёт, отёк, асимметрия, "
        "боль при глотании, охриплость, температура и признаки возможной инфекции."
    ),
    "ears": (
        "Фокус: ухо и область вокруг него. Оцени покраснение, отёк, выделения, боль, травму, "
        "признаки наружного отита, раздражения кожи и деформации ушной раковины."
    ),
    "lor": (
        "Фокус: ЛОР-область в целом. Оцени горло, миндалины, нос, носовое дыхание, уши, слух, "
        "голос, заложенность, выделения, боль при глотании, температуру и признаки инфекции."
    ),
    "dentistry": (
        "Фокус: стоматология и полость рта. Оцени зубы, дёсны, слизистую, налёт, кариозные дефекты, "
        "сколы, воспаление, отёк, боль при накусывании, запах, язвочки и признаки абсцесса."
    ),
    "surgery": (
        "Фокус: хирургические жалобы, послеоперационные швы, раны, перевязки, отёк, покраснение, "
        "мокнутие, выделения, расхождение краёв, гематома, серома, абсцесс и признаки инфицирования."
    ),
    "nails": (
        "Фокус: ногти, изменение цвета, формы, ломкость, расслоение, полосы, утолщение, воспаление вокруг ногтя, грибок."
    ),
    "hair": (
        "Фокус: кожа головы и волосы. Оцени очаги выпадения, ломкость, шелушение, зуд, покраснение, "
        "воспаление кожи головы и признаки возможной алопеции или дерматологической причины."
    ),
    "wounds": (
        "Фокус: раны, порезы, ссадины, ожоги, заживление, края раны, мокнутие, гной, кровотечение, "
        "покраснение вокруг, отёк и признаки инфицирования."
    ),
    "moles": (
...
CAN_ANALYZE = [
    ...
    "зубы, десны, налет, боль при накусывании",
    "ухо, ушная раковина, выделения, отёк",
    "ногти и кожа вокруг них",
    "волосы и кожа головы",
    "раны, порезы, ссадины, ожоги",
...
class HealthAnalyzeRequest(BaseModel):
    ...
    response_mode: Literal["short", "balanced", "detailed", "doctor", "surgeon", "lor", "dentistry"] = "balanced"
    messages: list[dict]
...
def _build_response_mode_instruction(response_mode: str) -> str:
    ...
    if response_mode == "doctor":
        return (
            "Формат ответа: для дерматолога. "
            "Пиши как краткую клиническую сводку без пациентских разъяснений. "
            "Структура ответа: 1) рабочее впечатление, 2) ключевые признаки, 3) дифференциальные варианты, "
    ...
    if response_mode == "lor":
        return (
            "Формат ответа: для ЛОР-врача. "
            "Пиши как короткую ЛОР-клиническую сводку по горлу, носу, уху, слуху и голосу. "
            "Обязательно оцени: боль при глотании, налёт, отёк, асимметрию миндалин, заложенность носа, "
            "выделения, снижение слуха, ушную боль, охриплость, температуру и признаки осложнений. "
            "Структура: 1) рабочее впечатление, 2) ключевые ЛОР-признаки, 3) вероятные ЛОР-диагнозы, "
            "4) красные флаги, 5) что уточнить при осмотре, 6) нужен ли срочный очный ЛОР-осмотр. "
            "Пиши профессионально, кратко и без лишнего общего текста."
        )
    if response_mode == "dentistry":
        return (
            "Формат ответа: для стоматолога. "
            "Пиши как короткую стоматологическую сводку по зубам, дёснам и слизистой полости рта. "
            "Обязательно оцени: кариозные дефекты, сколы, боль при накусывании, отёк, покраснение дёсен, "
            "кровоточивость, налёт, запах, язвочки, флюктуацию, абсцесс и признаки инфекции. "
            "Структура: 1) рабочее впечатление, 2) ключевые стоматологические признаки, 3) вероятные причины, "
            "4) красные флаги, 5) что уточнить при осмотре, 6) нужен ли срочный очный осмотр стоматолога. "
            "Формулируй коротко, по-деловому и клинически."
        )
