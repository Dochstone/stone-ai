...
import { getInitials, getAvatarColor, getSavedAvatar, saveAvatar, removeAvatar, processAvatarFile, syncAvatarFromProfile } from "@/lib/avatar";
      ...
      {avatar && !uploading && (
        <button
          onClick={(e) => {
            ...
            if (auth.token) {
              const API = process.env.NEXT_PUBLIC_API_URL || "https://stoneai.ru";
              fetch(`${API}/api/user/avatar`, { method: "DELETE", headers: { Authorization: `Bearer ${auth.token}` } }).catch(() => {});
            }
            removeAvatar();
            setAvatar(null);
          }}
