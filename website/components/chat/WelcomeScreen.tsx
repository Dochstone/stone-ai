  ...
  return (
    <div className="flex-1 flex items-start justify-center px-4 overflow-y-auto">
      <div className="text-center max-w-2xl w-full py-6 sm:py-8">
        <div className="mb-6">
          <img src="/mascots/stone-mascot-chat.webp?v=2" alt="Stone AI" width="72" height="72" className="mx-auto mb-3" />
            ...
            <div className="max-w-lg mx-auto text-left space-y-2">
              {[
                ...
                <button key={m.id} onClick={() => onSuggestion(m.prompt, m.id)}
                  ...
                  {(() => {
                    const company = MODELS.find(x => x.id === m.id)?.company;
                    const src = getModelIconSrc(m.id, company);
                    if (src) return <img src={src} alt={m.name} className="w-10 h-10 rounded-xl shrink-0 object-cover shadow-sm" loading="lazy" />;
                    return (
