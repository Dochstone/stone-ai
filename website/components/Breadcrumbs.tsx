    ...
    <>
      ...
      {!visualHidden && (
        <nav aria-label="Хлебные крошки" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <ol className="flex flex-wrap items-center gap-1.5 text-sm text-text/70">
            {all.map((item, i) => {
              const isLast = i === all.length - 1;
              return (
                <li key={item.href} className="flex items-center gap-1.5 min-w-0">
                  {isLast ? (
                    <span className="font-semibold text-text truncate max-w-[180px] sm:max-w-xs">
                      {item.label}
                    </span>
                  ) : (
                    <a
                      href={item.href}
                      className="text-text/70 hover:text-accent transition-colors truncate max-w-[120px] sm:max-w-[200px]"
                    >
                      {item.label}
                    </a>
                  )}
