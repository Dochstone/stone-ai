...
const VIDEO_MODEL_IDS = new Set([
  "sora-2", "veo-3", "luma-ray2", "luma-ray2-flash", "pika-2", "minimax", "pixverse-v5", "luma-dream", "wan-2", "hunyuan", "stable-video", "ltx-video", "kling-v2",
]);
...
// ─── Delete All Button (two-step confirm) ───
...
function DeleteAllButton({ onConfirm }: { onConfirm: () => void }) {
  ...
  return (
    <div className="px-2 py-2 shrink-0">
      {confirming ? (
        <div className="bg-red-500/8 border border-red-500/15 rounded-xl p-2.5 space-y-2">
          <p className="text-[11px] text-text/50 text-center">Удалить все чаты навсегда?</p>
          <div className="flex gap-2">
            <button
              onClick={() => { onConfirm(); setConfirming(false); }}
              className="flex-1 text-[11px] font-medium text-red-500 border border-red-500/20 hover:bg-red-500 hover:text-white py-1.5 rounded-lg transition-all"
            >
              Удалить
            </button>
            <button
              onClick={() => setConfirming(false)}
              className="flex-1 text-[11px] font-medium text-text/40 border border-text/[0.08] hover:bg-text/[0.06] py-1.5 rounded-lg transition-all"
            >
              Отмена
            </button>
          </div>
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-1.5 text-[11px] text-text/20 hover:text-text/40 py-1.5 rounded-lg transition-colors"
        >
  ...
  return (
    <>
      ...
      >
        ...
        {/* Delete all chats — two-step confirm */}
        {sessions.length > 1 && (
          <DeleteAllButton onConfirm={onDeleteAll} />
        )}
    ...
    try {
      ...
      const res = await fetch(chatUrl, {
        ...
        body: JSON.stringify({
          ...
          ...(activeBotConfig?.id && !isGuest ? { bot_id: activeBotConfig.id } : {}),
          ...(!activeBotConfig?.id && modelCatFilter === "health" ? { system_prompt: "Ты — AI-ассистент по общим вопросам здоровья. Ты НЕ врач и НЕ ставишь диагнозы. Анализируй фото и описания симптомов, предлагай возможные причины (2-3 варианта), рекомендуй к какому специалисту обратиться. Заверши: \"Для точного диагноза обратитесь к врачу.\" Отвечай на русском." } : {}),
        }),
        signal: abort.signal,
      });
