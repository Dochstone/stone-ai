...
export default function ToolHubPage({ params }: Props) {
  ...
  const baseFaqItems = [
    { q: `Сколько стоит ${hub.category.toLowerCase()}?`, a: `10 бесплатных запросов/день к текстовым моделям. Для генерации изображений — 2 бесплатные генерации. Подписки от ${planPriceFull("mini")} с доступом ко всем 65+ моделям.` },
  ...
  const extraFaqMap: Record<string, { q: string; a: string }[]> = {
    "image-generation": [
      ...
      { q: "Чем Stone AI лучше Midjourney для генерации картинок?", a: `Stone AI работает в браузере (не нужен Discord), поддерживает русский язык, предлагает 4 модели на выбор и стоит от ${planPriceFull("mini")}. Midjourney требует Discord и стоит от $10/мес.` },
    ],
    "video-generation": [
