...
export default function ProfessionPage({ params }: Props) {
  ...
  const howToStartMap: Record<string, { step: string; desc: string }[]> = {
    ...
    business: [
      ...
      { step: "Масштабируйте с подпиской", desc: "Подписка от 590₽/мес — доступ к GPT-5, Claude Opus, генерации картинок и видео. Дешевле одного часа работы фрилансера, но доступен 24/7." },
    ],
    teacher: [
  ...
  const resultsMap: Record<string, { metric: string; desc: string }[]> = {
    marketer: [
      { metric: "в 5 раз быстрее", desc: "создание контента: посты, статьи, email-рассылки" },
      { metric: "10+ часов/нед", desc: "экономия времени на рутинных задачах маркетинга" },
      { metric: "от 590₽/мес", desc: "вместо $20-100 за отдельные AI-инструменты" },
      { metric: "8 моделей бесплатно", desc: "для базовых задач без подписки каждый день" },
    ],
    developer: [
      ...
      { metric: "дешевле Copilot", desc: "от 590₽/мес вместо $19/мес за GitHub Copilot" },
    ],
    copywriter: [
    ...
    business: [
      ...
      { metric: "от 590₽/мес", desc: "дешевле одного часа фрилансера" },
