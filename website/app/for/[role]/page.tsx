...
export default function ProfessionPage({ params }: Props) {
  ...
  const benefitsMap: Record<string, string[]> = {
    ...
    developer: ["Функция на любом языке за 15 секунд — от Python до Rust", "Code review находит 15-20% багов, которые пропускает человек", "Тесты с edge-кейсами за 1 минуту — pytest, Jest, JUnit", "Дебаг по стек-трейсу: вставил ошибку → получил фикс", "Архитектурные решения с анализом trade-offs", "SQL-запрос из описания на русском за 10 секунд"],
    copywriter: ["Статья 2000 слов за 5 минут — с H2, списками и FAQ", "Рерайт с уникальностью 90-100% за 30 секунд", "Один текст → адаптация под VK, Telegram, Instagram за минуту", "SEO-оптимизация из коробки: ключи, мета-теги, структура", "Контент-план на месяц за 5 минут с темами и форматами", "Карточка товара для маркетплейса за 1 минуту"],
    designer: ["10 вариантов концепта за минуту — вместо часа скетчей", "Продуктовое фото без студии — смена фона за 15₽", "Текст на картинках — вывески, этикетки, мокапы с читаемым шрифтом", "Стилизация: flat, isometric, watercolor, comic — любой стиль", "Видео из текста — промо-ролик за 150₽ через Sora 2", "Логотип: 10 вариантов за 2 минуты для выбора направления"],
    student: ["10 бесплатных запросов каждый день — без карты", "Математика с пошаговым разбором — DeepSeek R1 (91.6% HumanEval)", "План курсовой с источниками — за 2 минуты", "Тест для ЕГЭ по любому предмету — за 1 запрос", "Объяснение сложной темы простым языком — сколько раз нужно", "Перевод + объяснение грамматики — AI-репетитор 24/7"],
    business: ["КП за 5 минут — с проблемой клиента, решением и ценами", "Анализ 5 конкурентов с рекомендациями — за 10 минут", "Карточки товаров для Ozon/WB — 15₽ за штуку, без дизайнера", "Скрипт продаж с обработкой возражений — за 2 минуты", "Договор-черновик по шаблону — за 3 минуты", "Контент для соцсетей без маркетолога — пост за 30 секунд"],
    teacher: ["План урока по ФГОС за 30 секунд — с целями и хронометражем", "Контрольная в 2 вариантах с ключами — за минуту", "Проверка сочинения с обратной связью — за 2 минуты", "Задания 3 уровней сложности по одной теме — за 1 запрос", "Кроссворды, квизы, интерактивные задания — за минуту", "10 бесплатных запросов каждый день — хватит на 2-3 урока"],
    smm: ["Пост с картинкой и хештегами — за 3 минуты вместо 30", "Контент-план на месяц — за 10 минут с темами и временем", "Reels-сценарий (hook + основа + CTA) — за 1 минуту", "Один текст → адаптация под VK, Telegram, Instagram, TikTok", "Ответы на негативные комментарии в тоне бренда — за 15 секунд", "2 бесплатные картинки каждый день через Nano Banana"],
  };
