...
const CATEGORY_FAQ: Record<string, { q: string; a: string }[]> = {
  chat: [
    { q: "Сколько запросов можно делать?", a: "Бесплатно (Pay-per-Use) — 10 быстрых + 2 премиум в день. На подписке Start — 600 быстрых + 150 премиум/мес, Pro — 1 500 быстрых + 112 премиум + 28 Opus/мес, Elite — 4 500 + 336 премиум + 56 Opus/мес + API." },
  ...
  image: [
    ...
    { q: "Сколько картинок можно сгенерировать?", a: "Pay-per-Use — 2 пробные, дальше от 3₽ с баланса. Start — 60/мес, Pro — 140/мес, Elite — 280/мес." },
  ],
  video: [
...
export function generateMetadata({ params }: Props): Metadata {
  ...
  return {
    title: `${model.name} на русском${isFree ? " бесплатно" : ""} — ${cat} онлайн без VPN 2026`,
    description: `Попробуйте ${model.name} от ${model.company} ${isFree ? "бесплатно" : `от ${planPrice("mini")}`} в России без VPN. ${model.description || ""} Контекст ${model.context}. Оплата картой РФ, СБП. Работает в браузере и Telegram.`,
