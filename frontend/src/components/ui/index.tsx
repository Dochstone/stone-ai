      ...
      {g && (
        <>
          <span style={{
            ...
            fontFamily: "'Orbitron',sans-serif", fontSize: size, fontWeight: 900,
            color: '#aaff00', clipPath: 'inset(15% 0 45% 0)',
          }}>{text}</span>
          <span style={{
            position: 'absolute', top: 0, left: -3,
            fontFamily: "'Orbitron',sans-serif", fontSize: size, fontWeight: 900,
            color: '#00ff88', clipPath: 'inset(55% 0 15% 0)',
          }}>{text}</span>
        </>
      )}
    </div>
...
export function MatrixRain({ primaryRgb, primary }: BgProps) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const p = primRef.current || '#00ff88'
      ...
      for (let i = 0; i < drops.length; i++) {
        ...
        const b = Math.random()
        if (b > 0.95) {
          ctx.fillStyle = p; ctx.shadowColor = p; ctx.shadowBlur = 15
        } else if (b > 0.8) {
          ctx.fillStyle = `rgba(${rgb},0.5)`; ctx.shadowBlur = 0
        } else {
          ctx.fillStyle = `rgba(${rgb},0.15)`; ctx.shadowBlur = 0
        }
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 0, pointerEvents: 'none', overflow: 'hidden' }}>
      ...
      <svg style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', opacity: 0.03 }} viewBox="0 0 480 800">
        {Array.from({ length: 12 }, (_, i) => (
          <line key={`h${i}`} x1="0" y1={i * 70} x2="480" y2={i * 70} stroke={`rgba(${primaryRgb},1)`} strokeWidth="0.5" />
        ))}
        {Array.from({ length: 8 }, (_, i) => (
          <line key={`v${i}`} x1={i * 70} y1="0" x2={i * 70} y2="800" stroke={`rgba(${secondaryRgb},1)`} strokeWidth="0.5" />
        ))}
      </svg>
    </div>
  )
  ...
  return (
    <div onClick={() => window.open('https://t.me/stoneAIC', '_blank')} style={{
      ...
      <span style={{
        ...
        color: primary, letterSpacing: '0.1em',
        textShadow: `0 0 12px rgba(${primaryRgb},0.5), 0 0 24px rgba(${primaryRgb},0.2)`,
      }}>STONE</span>
      <div style={{
          ...
          <button key={it.id} onClick={() => onNavigate(it.id)} style={{
            ...
            {it.main ? (
              <div style={{
                ...
                border: `2px solid ${isActive ? `rgba(${primaryRgb},0.5)` : `rgba(${primaryRgb},0.15)`}`,
              ...
              <span style={{
                fontSize: 20, opacity: isActive ? 1 : 0.35,
