...
const PAY_TABS = [
  ...
  { id: 'ton', label: '💎 TON', desc: 'Скоро' },
  { id: 'usdt', label: '💲 USDT', desc: 'Скоро' },
]
...
export function PlansScreen() {
  ...
  const handleBuy = async (item: any) => {
    ...
    if (payMethod !== 'stars') {
      showToast('Оплата TON/USDT скоро будет доступна', 'info')
      return
    }

    // Check if user already has this plan
    if (item.tier && user.plan === item.tier) {
      showToast(`У вас уже активна подписка ${item.tier.toUpperCase()}`, 'info')
      return
    }
    ...
    switch (result.status) {
      case 'success':
        showToast('Оплата прошла! Обновите приложение.', 'success')
      ...
      {user.plan !== 'free' && (
        ...
        >
          ...
          <span style={{ color: '#fff', fontSize: 13, fontWeight: 700 }}>
            Активна подписка {user.plan.toUpperCase()}
          </span>
        </div>
      )}
