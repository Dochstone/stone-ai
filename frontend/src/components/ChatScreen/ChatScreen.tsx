  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
      ...
      <div style={{
        ...
        <div onClick={() => setShowModels(!showModels)} style={{
        ...
        <div style={{ flex: 1, cursor: 'pointer' }} onClick={() => setShowModels(!showModels)}>
          ...
          <div style={{ fontFamily: 'monospace', fontSize: 10, color: '#5a8a70' }}>
            <span style={{ width: 5, height: 5, borderRadius: '50%', background: '#00ff88', display: 'inline-block', marginRight: 4, boxShadow: '0 0 6px #00ff88' }} />
            {currentModel?.company}
          </div>
        </div>
        <Tag text={currentModel?.tier === 'premium' ? 'PRO' : 'FREE'} accent={currentModel?.tier === 'premium' ? '#bf5af2' : '#00ff88'} />
      </div>
      ...
      {showModels && (
        <div style={{
          ...
          {models.map(md => {
            ...
            return (
              <div key={md.id} onClick={() => { if (!isLocked) { setModelId(md.id); setShowModels(false) } }} style={{
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 16px 120px' }}>
        ...
        {messages.map((msg, i) => (
          <div key={i} style={{
            ...
            <div style={{
              ...
              <div style={{ fontSize: 13, lineHeight: 1.55, color: '#e0f0e8', whiteSpace: 'pre-wrap' }}>{msg.content}</div>
              {msg.role === 'assistant' && (
        ...
        {isStreaming && messages.length > 0 && messages[messages.length - 1]?.content === '' && (
          <div style={{ display: 'flex', gap: 6, padding: '8px 0' }}>
            <div style={{
              background: 'rgba(8,16,12,0.9)', border: '1px solid rgba(0,255,136,0.08)',
              borderRadius: 14, padding: '10px 16px', display: 'flex', gap: 5,
            }}>
              {[0, 1, 2].map(j => (
                <div key={j} style={{
                  width: 6, height: 6, borderRadius: '50%', background: '#00ff88',
                  animation: `blink 1.2s ease-in-out ${j * 0.2}s infinite`, boxShadow: '0 0 6px #00ff88',
                }} />
              ))}
            </div>
          </div>
