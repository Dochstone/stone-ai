    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
      ...
      <div style={{
        ...
        <div style={{ flex: 1, cursor: 'pointer' }} onClick={() => setShowModels(!showModels)}>
          ...
          <div style={{ fontFamily: 'monospace', fontSize: 10, color: '#3a6a50' }}>
            <span style={{
              width: 5, height: 5, borderRadius: '50%', background: '#00ff88',
              display: 'inline-block', marginRight: 4, boxShadow: '0 0 6px #00ff88',
            }} />
      ...
      {showModels && (
        <div style={{
          ...
          {models.map(md => {
            ...
            return (
              <div key={md.id} onClick={() => {
                if (!isLocked) { setModelId(md.id); setShowModels(false) }
              }} style={{
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontFamily: 'monospace', fontSize: 10, color: '#3a6a50' }}>{md.company}</div>
                </div>
        ...
        {messages.map((msg, i) => (
          <div key={i} style={{
            ...
            <div style={{
              ...
              <div style={{
                fontSize: 13, lineHeight: 1.55, color: '#e0f0e8', whiteSpace: 'pre-wrap',
              }}>{msg.content}</div>
              {msg.role === 'assistant' && (
                <div style={{
                  fontFamily: 'monospace', fontSize: 9, color: '#1a3a28',
                  marginTop: 6, textAlign: 'right',
                }}>{currentModel?.icon} {currentModel?.name}</div>
              )}
            </div>
          </div>
        ))}
      ...
      <div style={{
        ...
        <div style={{
          ...
          <button onClick={handleSend} style={{
            ...
            background: input.trim() && !isStreaming ? 'rgba(0,255,136,0.15)' : 'rgba(0,255,136,0.04)',
            cursor: input.trim() && !isStreaming ? 'pointer' : 'default',
            fontSize: 16,
            color: input.trim() && !isStreaming ? '#00ff88' : '#1a3a28',
            transition: 'all 0.2s',
