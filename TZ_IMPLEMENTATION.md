...
<a id="этап-9"></a>
## Этап 9: Сайт — Страницы инструментов (по модели chatbotai.co)

### Цель
6 отдельных лендинг-страниц по инструментам. Каждая — SEO-оптимизирована, с описанием, подходящими моделями, примерами использования и CTA.

### Структура каждой страницы

```
1. Hero-блок с заголовком, описанием, CTA
2. «Как это работает» — 3 шага
3. Подходящие модели — карточки с ценами
4. Примеры использования / use-cases
5. FAQ по инструменту (3-4 вопроса)
6. CTA-блок
```
...
#### 9.1 `/chat` — AI Чат
- **Hero**: «Общайтесь с лучшими AI-моделями мира»
- **Модели**: GPT-5.1, Claude Opus 4, Gemini 2.5 Pro, DeepSeek R1, Grok 3 и бесплатные
- **Use-cases**: написание текстов, брейнсторм, анализ данных, помощь с задачами
- **FAQ**: "Чем отличаются модели?", "Какая модель лучше для текстов?", "Сохраняется ли история?"

#### 9.2 `/images` — Генерация картинок
- **Hero**: «Создавайте изображения с помощью AI»
- **Модели**: Nano Banana Pro, GPT-5 Image, GPT-5 Image Mini, Flux Schnell, SDXL, Nano Banana
- **Use-cases**: иллюстрации для соцсетей, аватарки, концепт-арт, мемы, товарные фото
- **Особенности**: показать примеры промптов и результатов (placeholder-блоки)
- **FAQ**: "Какие форматы?", "Можно ли редактировать?", "Сколько стоит одна картинка?"

#### 9.3 `/documents` — Анализ документов
- **Hero**: «Загрузите документ — AI ответит на любой вопрос»
- **Модели**: Claude Opus 4 (200K контекст), Gemini 2.5 Pro (1M контекст), GPT-4.1 (1M контекст)
- **Use-cases**: анализ PDF, сводки контрактов, выжимки из научных статей, фото документов
- **Особенности**: акцент на большие контексты моделей
- **FAQ**: "Какие файлы поддерживаются?", "Какой максимальный размер?", "Безопасно ли загружать документы?"

#### 9.4 `/search` — AI Поиск
- **Hero**: «Поиск в интернете с AI — актуальные данные в реальном времени»
- **Модели**: Perplexity Sonar, Perplexity Sonar Pro, Sonar Deep Research
- **Use-cases**: исследование рынка, проверка фактов, мониторинг новостей, конкурентный анализ
- **Особенности**: real-time данные, источники в ответе
- **FAQ**: "Откуда берутся данные?", "Чем отличается от обычного чата?", "Что такое Deep Research?"

#### 9.5 `/code` — Код-ассистент
- **Hero**: «AI-помощник для разработчиков»
- **Модели**: Claude Opus 4 / Sonnet 4, GPT-4.1, Devstral, DeepSeek R1, o3
- **Use-cases**: отладка, рефакторинг, генерация кода, code review, SQL-запросы
- **Особенности**: примеры с подсветкой синтаксиса (блоки кода)
- **FAQ**: "Какие языки поддерживаются?", "Можно ли загрузить файл?", "Какая модель лучше для кода?"

#### 9.6 `/translate` — Переводчик
- **Hero**: «Точный AI-перевод на 100+ языков»
- **Модели**: GPT-4o mini (бесплатный), Claude Haiku 4.5, Gemini 2.0 Flash — быстрые и дешёвые
- **Use-cases**: перевод документов, локализация, перевод с сохранением тона, адаптация маркетинговых текстов
- **Особенности**: акцент на понимание контекста vs Google Translate
- **FAQ**: "Чем лучше Google Translate?", "Какие языки?", "Можно ли перевести PDF?"
...
website/components/
├── ToolPageHero.tsx        # Общий hero для инструментов
├── ToolModels.tsx          # Блок подходящих моделей
├── ToolUseCases.tsx        # Use-cases карточки
└── ToolFaq.tsx             # FAQ-аккордеон (переиспользуемый)
```

- SSG (статические страницы) для SEO
- Метаданные (title, description, OG) для каждой страницы
- Навигация: добавить dropdown "Инструменты" в Nav
- Footer: добавить блок «Инструменты» со ссылками на все 6 страниц
...
| 9 | Сайт: 6 страниц инструментов | Этап 7 | 2-3 дня | 🟡 Важный |
